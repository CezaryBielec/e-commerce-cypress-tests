...
import { allFieldsValidation } from "../fixtures/validations";
...
describe("Sign up", () => {
    ...
    it('opens "CREATE AN ACCOUNT" page', () => {
      SignInPage.enterRandomEmailAddress().
      clickOnCreateAnAccount();

      SignUpPage.getUrl().should("contain", "account-creation");
    });
    ...
    .it('validates invalid email addres - value "%s"', (emailAddress) => {
        ...
        SignInPage.getValidation().
        should('contain', invalidEmailValidation);
    })

    it('validates email has already been used', () => {
        SignInPage.enterEmailAddress(emailOfAlreadyCreatedAccount).
        clickOnCreateAnAccount();
